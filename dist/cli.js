...
async function main() {
    ...
    if (command === "watch") {
        ...
        const stack = (stackFlagIdx !== -1 ? rest[stackFlagIdx + 1] : "react+tailwind");
        ...
        if (hasAnimation || gifPath) {
            ...
            await renderGif(data, { outputPath: gifPath }).catch((err) => {
    ...
    if (command === "scroll") {
        ...
        console.log(`   class mutations during scroll: ${data.mutationLog.filter((m) => m.isScrolling).length}`);
        ...
        if (data.mutationLog.filter((m) => m.isScrolling).length > 0) {
            console.log(`\n── Scroll-triggered class changes ──`);
            for (const m of data.mutationLog.filter((m) => m.isScrolling).slice(0, 15)) {
