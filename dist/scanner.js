...
export async function scan(url) {
    ...
    try {
        ...
        const candidates = await page.evaluate(() => {
            const seen = new Set();
            const results = [];
            const allCandidates = document.querySelectorAll('a, button, [role="button"], [class*="btn"], [class*="button"]');
            for (const el of Array.from(allCandidates)) {
                ...
                const hasTransition = computed.transitionDuration !== "0s" && computed.transitionProperty !== "none";
                ...
                if (el.id) {
                    ...
                    while (current && current !== document.body && depth < 5) {
                        ...
                        const classes = Array.from(current.classList).filter((c) => !/^(active|hover|focus|selected|open|is-|js-)/.test(c));
                        if (classes.length > 0) {
