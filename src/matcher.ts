...
export async function matchElement(
  ...
  .filter(w => w.length > 2 && !stopWords.has(w));  const textMatch = candidates.find(c =>
    words.every(w => c.text.toLowerCase().includes(w))
  );
  if (textMatch) return textMatch.selector;
...
Selector:`;
  ...
  if (!selector || selector === "NO_MATCH" || selector.length === 0) return null;
