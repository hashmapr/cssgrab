...
export interface WatchResult {
  selector: string;
  url: string;
  mode: 'element' | 'animation';
  extractedData?: any;
}

export async function watch(url: string): Promise<WatchResult> {
  ...
  let currentMode: 'element' | 'animation' = 'element';
  let resolved = false;
  ...
  page.on('popup', async (popup) => { try { await popup.close(); } catch { } });
  ...
  await page.route("**/*", (route) => {
    ...
    if (BLOCKED_HOST_FRAGMENTS.some(f => reqUrl.includes(f))) return route.abort();
  ...
  const resolvePromise = new Promise<void>((resolve) => {
    let pollInterval: ReturnType<typeof setInterval> | null = setInterval(async () => {
      ...
      try {
        ...
        if (title === '__CSSGRAB__SELECTMODE__OFF') return;
    ...
    const onData = (data: Buffer) => {
      ...
      if (str === '\r' || str === '\n') {
        ...
        pollInterval = null;
        resolved = true;
        try { process.stdin.setRawMode?.(false); process.stdin.removeListener('data', onData); } catch {}
        resolve();
      }
    };

    try { process.stdin.setRawMode?.(true); process.stdin.resume(); process.stdin.on('data', onData); } catch { }
  ...
  try {
    const finalSel = await page.evaluate(() => (window as any).__cssgrab_selected);
    if (finalSel) currentSelector = finalSel;
    const finalMode = await page.evaluate(() => (window as any).__cssgrab_mode);
    if (finalMode) currentMode = finalMode;
  } catch {}
  ...
  try {
    if (currentMode === 'animation') {
      ...
      extractedData = await page.evaluate(async (sel) => {
        ...
        const mutations: {selector: string; from: string; to: string}[] = [];
            ...
            if (r.type === 'attributes' && r.attributeName === 'style') {
              ...
              if (curr !== prev) {
                ...
                if (animProps.some(p => curr.includes(p) || prev.includes(p))) {
                  ...
                  if (el.className) s += '.' + Array.from(el.classList).slice(0, 2).join('.');
        ...
        for (const child of Array.from(root.querySelectorAll('*')).slice(0, 50)) {
          const s = (child as HTMLElement).style.cssText;
          if (s) snapshots.set(child, s);
        }

        await new Promise(r => setTimeout(r, 2000));
        mo.disconnect();

        const changes: {selector: string; before: string; after: string}[] = [];
        for (const [child, before] of snapshots.entries()) {
          const after = (child as HTMLElement).style.cssText;
          if (after !== before) {
            let s = (child as HTMLElement).tagName.toLowerCase();
            if ((child as HTMLElement).className)
              s += '.' + Array.from((child as HTMLElement).classList).slice(0, 2).join('.');
        ...
        const webAnimations = document.getAnimations()
          .filter(a => root.contains((a.effect as KeyframeEffect)?.target))
          .map((a: Animation) => {
      ...
      const [observedAnimations] = await Promise.all([
        page.evaluate(async (sel) => {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            if (s && !seen.has(s)) { seen.add(s); snapshots.push(s); }
          }
          return snapshots;
        }, currentSelector),
      ...
      extractedData = await page.evaluate((sel) => {
        ...
        const webAnimations = el.getAnimations
          ? el.getAnimations().map((a: Animation) => {
              const timing = a.effect?.getTiming?.() ?? {};
        ...
        return {
          ...
          classList: Array.from(el.classList).filter((c: string) => !c.startsWith('__cssgrab') && !c.startsWith('cssgrab')),
    ...
    console.warn(`\n  ⚠ Live extraction failed: ${(err as Error).message}`);
  }
