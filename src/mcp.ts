...
server.registerTool(
  "scan",
  {
    ...
    description:
      "Lists buttons, links, and other interactive elements on a webpage, " +
  ...
  async ({ url }) => {
    try {
      ...
      return {
        content: [{ type: "text" as const, text: `CSSgrab scan failed: ${message}` }],
        isError: true,
      };
    }
  },
...
server.registerTool(
  "grab",
  {
    ...
    description:
      "Extracts computed styles, source CSS, keyframes, Web Animations API " +
    ...
    inputSchema: {
      ...
      selector: z.string().describe(
        "CSS selector for the target element, e.g. '.btn-primary' or 'nav a:first-child'. " +
  ...
  async ({ url, selector }) => {
    try {
      ...
      if (data.isCanvas) {
        return {
          content: [{
            type: "text" as const,
            text:
              `This element (<${data.tag}> class="${data.classList.join(" ")}") is a ` +
      ...
      const message = err instanceof Error ? err.message : String(err);
      return {
        content: [{ type: "text" as const, text: `CSSgrab extraction failed: ${message}` }],
        isError: true,
      };
    }
  },
...
server.registerTool(
  "find",
  {
    ...
    description:
      "Scans a page and finds the best matching element for a natural language description. " +
  ...
  async ({ url, description }) => {
    try {
      ...
      if (!selector) {
        return { content: [{ type: "text" as const, text: `No element found matching: "${description}"` }] };
      }
      return { content: [{ type: "text" as const, text: selector }] };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      return { content: [{ type: "text" as const, text: `find failed: ${message}` }], isError: true };
    }
  },
);
server.registerTool(
  "scroll-scan",
  {
    ...
    description:
      "Scrolls the entire page from top to bottom while recording every animation " +
  ...
  async ({ url }) => {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      return {
        content: [{ type: "text" as const, text: `CSSgrab scroll-scan failed: ${message}` }],
        isError: true,
      };
    }
  },
...
server.registerTool(
  "deep-scan",
  {
    ...
    description:
      "Crawls an entire website (same domain, BFS) and runs scroll-scan on every page, " +
  ...
  async ({ url, maxDepth, maxPages, outputFile }) => {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      return {
        content: [{ type: "text" as const, text: `CSSgrab deep-scan failed: ${message}` }],
        isError: true,
      };
    }
  },
