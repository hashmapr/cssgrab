...
function paint(color: keyof typeof c, text: string) {
...
function printScanResults(candidates: ScanCandidate[]) {
...
function printHelp() {
  const row = (cmd: string, desc: string) =>
    `  ${paint("cyan", cmd.padEnd(30))} ${paint("dim", desc)}`;
...
async function cmdUse(args: string[]) {
  if (!args.length) { console.log(paint("red", "  Usage: use <url>")); return; }
  session.url = normaliseUrl(args[0]);
  console.log(paint("green", `  ✓ Active URL set to ${session.url}`));
}

async function cmdScan(args: string[]) {
  if (args.length) session.url = normaliseUrl(args[0]);
  if (!session.url) { console.log(paint("red", "  No URL set. Run: use <url>")); return; }
  ...
  try {
    ...
    stop();
    console.log(paint("red", `  Scan failed: ${(err as Error).message}`));
  }
}

async function cmdGrab(args: string[]) {
  if (!args.length) { console.log(paint("red", "  Usage: grab <n|selector> [url]")); return; }

  if (args[1]) session.url = normaliseUrl(args[1]);
  if (!session.url) { console.log(paint("red", "  No URL set. Run: use <url>")); return; }
  ...
  try {
    ...
    stop();
    console.log(paint("red", `  Grab failed: ${(err as Error).message}`));
    return;
  }

  // Stage 2 — element found
  console.log(
    `  ${paint("green", "✓")} ${paint("bold", `<${data.tag}>`)} ` +
  ...
  try {
    ...
    console.log("\n" + paint("red", `  Generation failed: ${(err as Error).message}`));
  }
}

function splitExplanation(raw: string): string {
...
async function cmdScroll(args: string[]) {
  ...
  try {
    ...
    console.log(`  ${paint("cyan", String(data.mutationLog.filter((m: any) => m.isScrolling).length).padStart(4))} scroll-triggered class mutations\n`);
    ...
    stop();
    console.log(paint("red", `  Scroll-scan failed: ${(err as Error).message}`));
  }
}

function cmdStack(args: string[]) {
...
async function dispatch(line: string) {
  ...
  switch (cmd) {
    ...
    case "grab":   await cmdGrab(args);   break;
    case "scroll": await cmdScroll(args); break;
    case "stack":  cmdStack(args);        break;
    case "url":
