...
type Provider = "ollama" | "anthropic" | "gemini" | "openrouter";

const PROVIDER = (process.env.LLM_PROVIDER as Provider) || "ollama";
...
async function askOllama(prompt: string): Promise<string> {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a precise code translator. You only use the data given " +
  ...
  if (!res.ok) throw new Error(`Ollama request failed (${res.status}). Is Ollama running locally?`);
